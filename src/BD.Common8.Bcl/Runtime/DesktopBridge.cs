...
public abstract class DesktopBridge
{
    ...
    /// <para></para>
    /// 与 OperatingSystem2.IsRunningAsUwp 不同，OperatingSystem2.IsRunningAsUwp 通过判断进程含有包标识返回 <see cref="bool"/>，从 Windows 10 版本 2004 开始，可以通过生成稀疏包并将其注册到应用，向未打包到 MSIX 包中的桌面应用授予包标识符。
