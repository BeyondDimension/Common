...
public static partial class ComponentCodeExtensions
{
    ...
    /// <returns></returns>
    public static async Task ToggleStateCoreAsync(this IDisableLoading item, IApiConnection api, string apiRelativeUrl, CancellationToken cancellationToken = default)
    {
        ...
        where TPrimaryKey : notnull, IEquatable<TPrimaryKey>
    {
        ...
        if (rsp.IsSuccess)
        {
            ...
            {
                ShowSuccess();
                async void ShowSuccess() => await message.Success(isAdd ? Strings.AddSuccess : Strings.EditSuccess);
            }
    ...
    public static void AddOrEditCore<TEditDTO, TPrimaryKey>(this IAddOrEditModal<TEditDTO, TPrimaryKey> thiz, KeyModel<TPrimaryKey>? item, string? title = null)
        ...
        where TPrimaryKey : notnull, IEquatable<TPrimaryKey>
    {
        thiz.AddOrEditModalTitle = title ?? (item == null ? Strings.Add : Strings.Edit);
    ...
    public static async Task DeleteCoreAsync(this IDeleteLoading item, ITableQuery query, IApiConnection api, string apiRelativeUrl, ModalService modal, MessageService? message, string content, CancellationToken cancellationToken = default)
    {
        var isOK = await modal.ConfirmAsync(new ConfirmOptions
        {
            ...
            Content = string.Format(Strings.ConfirmContentDelete_, content),
            OkType = "danger",
            Width = 520D,
        });
        if (!isOK) return;
        ...
        if (rsp.IsSuccess)
        {
            ...
            {
                ShowSuccess();
                async void ShowSuccess() => await message.Success(Strings.DeleteSuccess);
            }
