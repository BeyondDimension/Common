...
try
{
    ...
    foreach (var item in appSettings.Settings)
    {
        if (!string.IsNullOrWhiteSpace(item.Value.ClassFilePath))
        {
            ...
            using var stream = new FileStream(classFilePath, FileMode.OpenOrCreate, FileAccess.ReadWrite, FileShare.ReadWrite | FileShare.Delete);
        ...
        if (!string.IsNullOrWhiteSpace(item.Value.InterfaceFilePath))
        {
            ...
            using var stream = new FileStream(interfaceFilePath, FileMode.OpenOrCreate, FileAccess.ReadWrite, FileShare.ReadWrite | FileShare.Delete);
