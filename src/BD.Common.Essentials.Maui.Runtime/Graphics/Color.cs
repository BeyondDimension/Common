...
public class Color
{
    ...
    public static bool TryParse(string value, [NotNullWhen(true)] out Color? color) => TryParse(value != null ? value.AsSpan() : default, out color);

    static bool TryParse(ReadOnlySpan<char> value, [NotNullWhen(true)] out Color? color)
    {
        value = value.Trim();
        if (!value.IsEmpty)
        {
