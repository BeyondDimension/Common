...
/// <summary>
/// <see cref="IBMMenuRepository"/> 的实现类
/// </summary>
/// <typeparam name="TDbContext"></typeparam>
/// <remarks>
/// Initializes a new instance of the <see cref="BMMenuRepository{TDbContext}"/> class.
/// </remarks>
/// <param name="mapper"></param>
/// <param name="dbContext"></param>
/// <param name="requestAbortedProvider"></param>
sealed class BMMenuRepository<TDbContext>(IMapper mapper, TDbContext dbContext, IRequestAbortedProvider requestAbortedProvider) : Repository<TDbContext, BMMenu, Guid>(dbContext, requestAbortedProvider), IBMMenuRepository where TDbContext : DbContext, IBMDbContext
{
    ...
    static readonly Comparison<BMMenu> menuComparisonByOrder = (a, b) => (int)(a.Order - b.Order);

    public async Task<BMMenuTreeItem[]> GetTreeAsync()
    {
        ...
        return mapper.Map<BMMenuTreeItem[]>(menus);
    }

    public async Task<BMMenuModel2?> InfoAsync(Guid id)
    {
        var r = await Entity.AsNoTrackingWithIdentityResolution()
            .Select(BMMenuModel2.Expression)
    ...
    public async Task<bool> EditMenuButtonsAsync(Guid menuId, Guid[] buttons, Guid tenantId)
    {
        ...
        await db.MenuButtons.AddRangeAsync(buttons.Select(x => new BMMenuButton
        {
            MenuId = menuId,
            ButtonId = x,
            TenantId = tenantId,
    ...
    public static BMMenuModel? MapToTreeModel(Dictionary<Guid, IEnumerable<BMButtonModel>>? dic, BMMenu item)
    {
        if (dic == null || !dic.TryGetValue(item.Id, out var sysButton))
            return null;
        if (sysButton == null)
            return null;
        var destination = new BMMenuModel()
        {
            Id = item.Id,
            Key = item.Key,
            Name = item.Name,
    ...
    public BMButtonModel? GetMenuSysButtonModel(Dictionary<Guid, BMButtonModel> dic, Guid buttonId)
    {
    ...
    public async Task<BMMenuModel2[]> GetRoleTreeAsync()
    {
        var query = Entity.AsNoTrackingWithIdentityResolution()
            .Include(x => x.Children)
    ...
    public async Task<BMButtonModel[]> GetButtonsAsync()
    {
        var query = db.Set<BMButton>()
            .AsNoTrackingWithIdentityResolution()
            .Where(x => !x.SoftDeleted);
        var r = await query
            .Select(BMButtonModel.Expression)
            .ToArrayAsync();
        return r;
    }
    ...
    public async Task<BMButtonModel[]> GetRoleMenuButtonsAsync(
        Guid roleId,
        Guid menuId,
        Guid tenantId)
    {
...
#endif
        var r = await query
            .Select(BMButtonModel.Expression).ToArrayAsync();
        return r;
    }
    ...
    public async Task<Dictionary<Guid, BMButton>> GetButtons(Guid? tenantId)
    {
