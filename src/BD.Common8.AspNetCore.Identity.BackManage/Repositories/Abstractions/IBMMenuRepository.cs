...
public interface IBMMenuRepository
{
    /// <summary>
    /// 查询树结构
    /// </summary>
    ...
    /// <returns></returns>
    Task<BMMenuTreeItem[]> GetTreeAsync();
    ...
    /// <param name="id"></param>
    /// <returns></returns>
    Task<BMMenuModel2?> InfoAsync(Guid id);
    ...
    /// <param name="tenantId"></param>
    /// <returns></returns>
    Task<BMMenuModel[]?> GetUserMenuAsync(Guid userId, Guid tenantId);

    /// <summary>
    /// 获取角色权限编辑菜单（暂时没有加基础菜单表等无筛选权限）
    /// </summary>
    /// <returns></returns>
    Task<BMMenuModel2[]> GetRoleTreeAsync();

    /// <summary>
    /// 获取系统按钮列表
    /// </summary>
    /// <returns></returns>
    Task<BMButtonModel[]> GetButtonsAsync();
    ...
    /// <param name="tenantId"></param>
    /// <returns></returns>
    Task<BMButtonModel[]> GetRoleMenuButtonsAsync(
        Guid roleId,
        Guid menuId,
        Guid tenantId);
    ...
    /// <param name="tenantId"></param>
    /// <returns></returns>
    Task<BMMenuModel[]?> GetUserMenu2Async(Guid userId, Guid tenantId);
