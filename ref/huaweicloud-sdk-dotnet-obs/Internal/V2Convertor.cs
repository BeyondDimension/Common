    ...
    internal class V2Convertor : IConvertor
    {
        ...
        protected void TransPutObjectBasicRequest(PutObjectBasicRequest request, HttpRequest httpRequest)
        {
            ...
            foreach (KeyValuePair<ExtensionObjectPermissionEnum, IList<string>> map in request.ExtensionPermissionMap)
            {
                ...
                switch (map.Key)
                {
                    case ExtensionObjectPermissionEnum.GrantFullControl: header = iheaders.GrantFullControlHeader(); break;
                    case ExtensionObjectPermissionEnum.GrantRead: header = iheaders.GrantReadHeader(); break;
                    case ExtensionObjectPermissionEnum.GrantReadAcp: header = iheaders.GrantReadAcpHeader(); break;
                    case ExtensionObjectPermissionEnum.GrantWriteAcp: header = iheaders.GrantWriteAcpHeader(); break;
                    default: break;
                }
                if (!string.IsNullOrEmpty(header) && map.Value != null && map.Value.Count > 0)
                {
        ...
        {
            HttpRequest httpRequest = Trans(request as PutObjectRequest);
        ...
        public HttpRequest Trans(CopyPartRequest request)
        {
            ...
            {
                CommonUtil.AddHeader(httpRequest, iheaders.CopySourceRangeHeader(), string.Format("bytes={0}-{1}", request.ByteRange.Start, request.ByteRange.End));
            }
        ...
        {
            HttpRequest httpRequest = Trans(request as GetObjectMetadataRequest);
